...
func SetupTestServer(t *testing.T) (*gin.Engine, *gorm.DB, func()) {
	...
	testModels := []interface{}{
		&model.Patient{}, &model.Disease{}, &model.User{}, &model.Session{}, &model.Therapist{}, &model.Role{}, &model.Treatment{}, &model.Transaction{}, &model.PatientCode{}, &model.Employee{}, &model.Expense{},
	}
...
func CreateAndLoginUser(t *testing.T, r http.Handler, creds SignupCreds) (string, uint) {
	...
	rr, err := doRequest(r, requestParams{method: "POST", path: "/signup", body: b})
	if err != nil {
	...
	rr, err = doRequest(r, requestParams{method: "POST", path: "/login", body: b})
	if err != nil {
...
func ListUsersData(t *testing.T, r http.Handler, adminToken string, query string) map[string]interface{} {
	...
	rr, err := doRequest(r, requestParams{method: "GET", path: path, body: nil, headers: map[string]string{"session-token": adminToken}})
	if err != nil {
...
func PatchUserEmail(t *testing.T, r http.Handler, req EmailUpdateRequest) *httptest.ResponseRecorder {
	...
	rr, err := doRequest(r, requestParams{method: "PATCH", path: path, body: b, headers: map[string]string{"session-token": req.Token}})
	if err != nil {
